...
def active_students():
    ...
    # Define active students as those not in dropout category
    score_cols = ['math_score', 'reading_score', 'writing_score']
    existing_score_cols = [col for col in score_cols if col in df.columns]
    ...
    df['overall_score'] = df[existing_score_cols].mean(axis=1)
    active_students_df = df[df['overall_score'] >= 40] # Not dropout
    return jsonify({"active_students": len(active_students_df)})